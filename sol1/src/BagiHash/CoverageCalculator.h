...
{
private:
	inline static unsigned int determine_coverage_position(const Data& data, const unique_ptr<unique_ptr<unsigned int[]>[]>& nr_walls,
		Point position)
	{
		const auto compute_rect_sum = [&nr_walls](Matrix matrix)
		{
			// rectangle [(0, 0), (i2, j2)]
		...
		const unsigned int i_min = (unsigned int)max((long long)position.first - (long long)data.router_radius, 0ll);
		const unsigned int j_min = (unsigned int)max((long long)position.second - (long long)data.router_radius, 0ll);
		const unsigned int i_max = min(position.first + data.router_radius, data.nr_rows - 1);
		const unsigned int j_max = min(position.second + data.router_radius, data.nr_columns - 1);
		...
		for (unsigned int i = i_min; i <= i_max; ++i)
			for (unsigned int j = j_min; j <= j_max; ++j)
			{
				...
				{
					const Point upper_left = make_pair(min(i, position.first), min(j, position.second));
					const Point lower_right = make_pair(max(i, position.first), max(j, position.second));
	...
	static void determine_coverage_split(const Data& data, unique_ptr<unique_ptr<unsigned int[]>[]>& coverage, const unique_ptr<unique_ptr<unsigned int[]>[]>& nr_walls,
		Matrix matrix)
	{
		for (unsigned int i = matrix.first.first; i <= matrix.second.first; ++i)
			for (unsigned int j = matrix.first.second; j <= matrix.second.second; ++j)
				if (data.building_plan[i][j] != '#')
					coverage[i][j] = determine_coverage_position(data, nr_walls, make_pair(i, j));
				else
					coverage[i][j] = 0;
	}

	static array<Matrix, NR_SUBMATR> split_matrix(const Data& data)
	{
...
public:
	...
	static unique_ptr<unique_ptr<unsigned int[]>[]> determine_coverage(const Data& data)
	{
		// Allocating memory
		auto nr_walls = make_unique<unique_ptr<unsigned int[]>[]>(data.nr_rows);
		...
		auto sub_matrices = split_matrix(data);

		array<thread, NR_SUBMATR> threads;
		for (unsigned int index = 0; index < NR_SUBMATR; ++index)
			threads[index] = thread(determine_coverage_split, cref(data), ref(coverage), cref(nr_walls), sub_matrices[index]);
