...
private:
	...
	map<Point, Point> apply_Lee(Point initial_router)
	{
		...
		map<Point, Point> routers_to_parents;
		routers_to_parents.insert(make_pair(initial_router, data.initial_cell));
...
public:
	...
	vector<map<Point, Point>> get_components()
	{
		vector<map<Point, Point>> components;
		for (unsigned int i = 0; i < data.nr_rows; ++i)
			for (unsigned int j = 0; j < data.nr_columns; ++j)
